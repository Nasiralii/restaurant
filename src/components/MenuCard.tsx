...
export function MenuCard({ item }: { item: MenuItem }) {
  ...
  // Use the key field directly - no complex logic needed
  const getItemTranslation = (field: 'name' | 'description') => {
    if (!item.key) return item[field]; // Fallback if no key
    const translation = t(`items.${item.key}.${field}`);
    // Simple check: if translation is same as key, return original
    if (translation === `items.${item.key}.${field}`) {
      return item[field];
    }
    return translation;
  };
  ...
  return (
    <article className="group rounded-2xl border border-black/10 bg-[#fdf7f1] p-5 shadow-sm backdrop-blur transition hover:-translate-y-1 hover:shadow-lg hover:border-[#c28a52]/60">
      <div className="relative mb-4 overflow-hidden rounded-xl bg-[#f2e0c8]">
        ...
        <div className="pointer-events-none absolute inset-0 bg-linear-to-t from-black/45 via-black/10 to-transparent opacity-0 transition duration-300 ease-out group-hover:opacity-100" />
      </div>
